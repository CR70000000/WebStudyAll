...
document.querySelector('.next').addEventListener('click', e => {
  // 如果当前页数小于总页数，则将当前页数加1，并调用setArtileList函数
  if (queryQbj.page < Math.ceil(totalCount / queryQbj.per_page)) {
    queryQbj.page++
    document.querySelector('.page-now').innerHTML = `第${queryQbj.page}页`
    setArtileList()
  }
})
// 3.3 点击上一页，做临界值判断，并切换页码参数并请求最新数据
document.querySelector('.last').addEventListener('click', e => {
  // 如果当前页数大于1，则将当前页数减1，并调用setArtileList函数
  if (queryQbj.page > 1) {
    queryQbj.page--
    document.querySelector('.page-now').innerHTML = `第${queryQbj.page}页`
    setArtileList()
...
document.querySelector('.art-list').addEventListener('click', async e => {
  ...
  if (e.target.classList.contains('del')) {
    ...
    if (children.length === 1 && queryQbj.page !== 1) {
      queryQbj.page--
      document.querySelector('.page-now').innerHTML = `第${queryQbj.page}页`
    }
    // 4.4 重新获取文章列表，并覆盖展示
